...
export function HeroLoader({ progress, ready, onExited }: HeroLoaderProps) {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 grid place-items-center bg-black p-5 transition-opacity duration-[400ms] ${leaving ? 'pointer-events-none opacity-0' : 'opacity-100'}`}
      role="status"
      aria-live="polite"
    >
      <div className="w-full max-w-sm border-4 border-black bg-[#f5f1e8] p-6 text-center shadow-[8px_8px_0_0_#000]">
        <p className="mb-3 -rotate-1 bg-black px-3 py-2 text-sm font-black uppercase tracking-[0.18em] text-[#ffe500]">
          Preparing the party
        </p>
        <DotLottiePlayer
          src="/loading.lottie"
        ...
        <div className="mt-5 border-4 border-black bg-white p-1">
          <div
            className="h-5 bg-[#00d9ff] transition-[width] duration-150"
